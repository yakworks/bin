...
# arg $1 - the env
# arg $2 - the db
# arg $3 - USE_BUILDER true to use the docker builder
...
# arg $3 - USE_BUILDER true to use the docker buildr
function initEnv {
  setVar BUILD_ENV ${1:-test}
  setDbEnv $2
  [ "$3" = true ] && USE_BUILDER=true
  # build and env vars
...
function setupDockerExecs {
  ...
  # if USE_BUILDER then set DockerExec which is what make will try and use
...
function setDbEnv {
  ...
  if [ "$USE_BUILDER" = "true" ] || [ -f /.dockerenv ] && [ "$CI" != "true" ]; then
